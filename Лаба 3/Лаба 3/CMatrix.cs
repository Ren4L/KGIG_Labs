...
namespace Лаба_3
{
    public class CMatrix
    {
        ...
        {
            double[] arr = { };
            Array.Resize(ref arr, this.Row);
            for (int i = 0; i < this.Row; i++)
            {
                arr[i] = this.Matrix[i][c];
            }
            return arr;
        }
    ...
    static class CMatrixMethod
    {
        ...
        public static CMatrix SphereToCart(CMatrix PView)
        {
            if (PView.column == 1)
            {
                return new CMatrix((PView[0, 0] * Math.Cos(PView[1, 0]) * Math.Sin(PView[2, 0])), (PView[0,0] * Math.Sin(PView[1,0]) * Math.Sin(PView[2,0])), (PView[0,0] * Math.Cos(PView[2,0])));
            }
